                        ...
                        <form action="{{ route('periksa.update', $periksa->id) }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <select id="id_pasien" name="id_pasien"
                                    class="form-select @error('id_pasien') is-invalid @enderror" required>
                            ...
                            <div class="mb-3">
                                ...
                                <select id="id_dokter" name="id_dokter"
                                    class="form-select @error('id_dokter') is-invalid @enderror" required>
                            ...
                            <div class="mb-3">
                                ...
                                <select id="id_obat" name="id_obat"
                                    class="form-select @error('id_obat') is-invalid @enderror" required>
                                    <option value="">-- Pilih Obat --</option>
                                    @foreach ($obat as $item)
                                        <option value="{{ $item->id }}"
                                            {{ old('id_obat', optional($periksa->detailPeriksa->first()?->obat)->id) == $item->id ? 'selected' : '' }}>
                                            {{ $item->name_obat }}
                                        </option>
                                    @endforeach
                                </select>
